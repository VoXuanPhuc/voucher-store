import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { of } from 'rxjs';

import { FeedbackImageService } from '../service/feedback-image.service';

import { FeedbackImageComponent } from './feedback-image.component';

describe('Component Tests', () => {
  describe('FeedbackImage Management Component', () => {
    let comp: FeedbackImageComponent;
    let fixture: ComponentFixture<FeedbackImageComponent>;
    let service: FeedbackImageService;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [HttpClientTestingModule],
        declarations: [FeedbackImageComponent],
      })
        .overrideTemplate(FeedbackImageComponent, '')
        .compileComponents();

      fixture = TestBed.createComponent(FeedbackImageComponent);
      comp = fixture.componentInstance;
      service = TestBed.inject(FeedbackImageService);

      const headers = new HttpHeaders().append('link', 'link;link');
      jest.spyOn(service, 'query').mockReturnValue(
        of(
          new HttpResponse({
            body: [{ id: 123 }],
            headers,
          })
        )
      );
    });

    it('Should call load all on init', () => {
      // WHEN
      comp.ngOnInit();

      // THEN
      expect(service.query).toHaveBeenCalled();
      expect(comp.feedbackImages?.[0]).toEqual(expect.objectContaining({ id: 123 }));
    });
  });
});
